...
namespace topology::grid::tests {
    struct test_grid {
        ...
        void perform_tests() {
            ...
            auto ij_2_idx = IJIndexer(width_, height_);
            NodeArray<TNode> grid = make_grid<TNode, NodeExecutorMock>(width_, height_, nullptr, topology_);
            ...
            for (Index i = 0; i != height_; ++i)
                for (Index j = 0; j != width_; ++j) {
                    ...
                    auto foo = ij_2_idx(i, j);
                    TNode *node = grid[ij_2_idx(i, j)];
                    node->executor()->exec();
